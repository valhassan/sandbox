...
def main(csv_file):
    ...
    for raster_str in tqdm.tqdm(tiff_column):
        ...
        out_pth = f"{raster_path.parent}/{raster_path.stem.split('_')[0]}_RGB_8bit.tif"
        gdal_translate_cmd = f"gdal_translate -b 3 -b 2 -b 1 -ot Byte -of GTiff {raster_path} {out_pth}"
        os.system(gdal_translate_cmd)
        dir_column.append(out_pth)
    df[df.columns[0]] = dir_column
    out_file = in_path.parent.joinpath('planetscope_trn_RGB.csv')
    df.to_csv(out_file, index=False, header=False)
    logging.info(f' Complete!!! saved to {out_file}')
    logging.info(f'BGR to RGB Complete!!!')
